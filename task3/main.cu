/**
 * Zpracovani obrazu pomoci 2D konvoluce.
 */
#include <stdio.h>
#include <stdlib.h>
#include <cuda.h>
...
int main(int argc, char **argv) {
    ...
    char in_file_name[] = "../lena.png";
    char out_file_name[30] = "../lena_";

    if (argc == 2) { // [program name] [filter name]

        filter = argv[1];
        strcat(out_file_name, filter);
        strcat(out_file_name, ".png");

        if ((strcmp(filter, "blur") == 0) || (strcmp(filter, "edges") == 0) || (strcmp(filter, "sharpen") == 0) || (strcmp(filter, "emboss") == 0) || (strcmp(filter, "copy") == 0)) {
    ...
    // Copy filter matrix to graphic card.
    ...
    // Allocate image buffer on graphic card.
    ...
    // Copy raw buffer from host memory to graphic card memory.
    ...
    // Copy raw buffer from graphic card back to host.
